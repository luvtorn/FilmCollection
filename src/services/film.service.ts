...
import { IFilm, IFilmResponse, IGenres, IResult } from '../types'
...
//try, catch сделать, film api service
...
class FilmsService {
  ...
  async getMainPageFilms(): Promise<IFilmResponse> {
    try {
      const url =
        'https://api.themovedb.org/3/movie/upcoming?language=en-US&page=1'
      const { data } = await axios.get<IFilmResponse>(url, options)
      return data
    } catch (err) {
      console.log(err)
    }
  ...
  async getFilmsByKeyword(str: string, page: number): Promise<IFilm> {
    const url = `https://api.themoviedb.org/3/search/movie?query=${str}&page=${page}`
    const { data } = await axios.get<IFilm>(url, options)
    return data
  }
